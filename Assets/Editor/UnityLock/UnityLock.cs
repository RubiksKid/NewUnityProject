// Original idea & developer : Nick Gravelyn (2012)
// Improvements to lock/Unlock directly in the hierarchy window : Julian Oliden 'Jocyf' - 2013.
...
public class UnityLock : EditorWindow
{
    ...
    private static class HierarchyIcon
    {
		...
		private static Texture2D _iconUnLock;
        ...
        private static string iconFile
        {
		 ...
		 private static string iconFileUnLock
        {
            get
            {
                return "Assets" + Directory.GetFiles(Application.dataPath, "UnityLockHierarchyIconUnLock.png", SearchOption.AllDirectories)[0].Substring(Application.dataPath.Length).Replace('\\', '/');
            }
        }
        ...
        private static void OnHierarchyWindowItemOnGUI(int instanceID, Rect selectionRect)
        {
            ...
            if ((obj.hideFlags & HideFlags.NotEditable) == HideFlags.NotEditable)
            {
                ...
                if(GUI.Button(new Rect(selectionRect.xMax - _iconSize, selectionRect.center.y - (_iconSize / 2f), _iconSize, _iconSize), _icon, GUIStyle.none)){
					if (Event.current.button == 0)
						UnlockObject((GameObject)obj);
			...
			//if (obj )
				if (!_iconUnLock)
                {
                    _iconUnLock = AssetDatabase.LoadAssetAtPath(iconFileUnLock, typeof(Texture2D)) as Texture2D;
                }

                if(GUI.Button(new Rect(selectionRect.xMax - _iconSize, selectionRect.center.y - (_iconSize / 2f), _iconSize, _iconSize), _iconUnLock, GUIStyle.none)){
					if (Event.current.button == 0)
						LockObject((GameObject)obj);
					else if (Event.current.button == 1)
						LockRecursively();
				}
			}
